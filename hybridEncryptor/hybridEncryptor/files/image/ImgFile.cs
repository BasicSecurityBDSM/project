    ...
    class ImgFile : File
    {
        ...
        public override void Generate(byte[] dataToInsert)
        {
            ...
            for (int i = 0; i < pixels.Length; i+=4)
            {
                ...
                holder ^= 1;
                pixels[i] &= holder;
                pixels[i] ^= bitsToInsert[i/4];
            }
            unsafe
            {
                fixed (byte* ptr = pixels)
                {
                    image = new Bitmap(width, height, stride, PixelFormat.Format32bppRgb, new IntPtr(ptr));
                }
            }
            image = embedText(Convert.ToBase64String(dataToInsert), (Bitmap)image);
        }

        public override byte[] GetData()
        {
            return Convert.FromBase64String(extractText((Bitmap)image));
        }
        ...
        /// <summary>
        /// alles wat volgt komt van : http://www.codeproject.com/Tips/635715/Steganography-Simple-Implementation-in-Csharp
        /// </summary>
        public enum State
        {
            Hiding,
